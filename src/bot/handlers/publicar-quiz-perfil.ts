...
import { examplePerfilQuizTemplate } from "@barqueiro/types";
import {
  getPerfilQuizConfig,
  isPerfilQuizReady,
  setPerfilQuizConfig,
...
/**
 * Handler para o subcomando publicar do quiz de perfil
 */
export async function handlePublicarQuizPerfil(
  ...
  // Verificar se o quiz está configurado
  const isReady = await isPerfilQuizReady();
  const config = await getPerfilQuizConfig();

  if (!isReady) {
    // Oferecer interface interativa para configurar
    await mostrarConfigInicialQuiz(interaction);
    return;
  }

  // Mostrar interface de publicação
  await mostrarInterfacePublicacao(interaction, config);
}

/**
 * Interface para configuração inicial do quiz
 */
async function mostrarConfigInicialQuiz(
  interaction: ChatInputCommandInteraction,
): Promise<void> {
  const embed = new EmbedBuilder()
    .setTitle("⚙️ Quiz de Perfil Não Configurado")
    .setDescription(
      "O quiz de perfil não está configurado ainda. Escolha uma opção abaixo para começar:",
    )
    .addFields([
      {
        name: "🚀 Configuração Rápida (Recomendado)",
        value:
          "Ativa um quiz básico com:\n" +
...
/**
 * Interface interativa para publicação do quiz
 */
async function mostrarInterfacePublicacao(
  interaction: ChatInputCommandInteraction,
  ...
  // Seleção de perguntas (índices) - por padrão todas
  let selectedQuestionIdx: number[] = config.questions.map((_, idx) => idx);
      ...
      .addFields([
        {
          ...
          value:
            `• **Questões selecionadas:** ${selectedQuestionIdx.length}/${config.questions.length}\n` +
  ...
  collector.on("collect", async (i) => {
    ...
    // Preview do quiz
      ...
      const previewEmbed = new EmbedBuilder()
        ...
        .addFields(
          config.questions.slice(0, 3).map((q, idx) => ({
    ...
    // Publicar quiz
      ...
      // Atualizar canal na config
      config.channelId = targetChannelId;
      await setPerfilQuizConfig(config);

      // Publicar quiz
      await publicarMensagemQuiz(
        targetChannel as { send: (options: unknown) => Promise<unknown> },
        {
          ...config,
  ...
  // Coletor para menu de seleção de canal
...
/**
 * Publica a mensagem do quiz no canal
 */
async function publicarMensagemQuiz(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Crie seu perfil personalizado respondendo algumas perguntas!**\n\n" +
        "Este quiz irá:\n" +
...
/**
 * Handler para configurações do quiz de perfil
 */
export async function handleConfigQuizPerfil(
  interaction: ChatInputCommandInteraction,
): Promise<void> {
  // Verificar permissões de administrador
  if (!interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) {
...
/**
 * Exibe status atual do quiz
 */
async function handleStatusQuiz(
  interaction: ChatInputCommandInteraction,
  config: PerfilQuizConfig,
): Promise<void> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**Estado:** ${config.enabled ? "🟢 Ativo" : "🔴 Inativo"}\n` +
        `**Canal:** ${canal}\n` +
        `**Questões:** ${config.questions.length}\n` +
        `**Resultados:** ${config.results.length}\n` +
    ...
    .addFields([
      {
        ...
        value:
          "• `/publicar-quiz-perfil config toggle` - Ativar/Desativar\n" +
...
/**
 * Configura cor padrão dos perfis
 */
async function handleCorPadrao(
  interaction: ChatInputCommandInteraction,
  config: PerfilQuizConfig,
  valor: string | null,
): Promise<void> {
  ...
  // Aplicar cor a todos os resultados que não têm cor específica
  config.results.forEach((result: PerfilQuizResult) => {
    if (!result.aparencia) {
...
/**
 * Lista resultados possíveis do quiz
 */
async function handleListarResultados(
  interaction: ChatInputCommandInteraction,
  config: PerfilQuizConfig,
): Promise<void> {
  ...
  config.results
    .slice(0, 10)
    .forEach((result: PerfilQuizResult, index: number) => {
      ...
      embed.addFields([
        {
          ...
          value:
            `**Bio:** ${result.bio.slice(0, 50)}${result.bio.length > 50 ? "..." : ""}\n` +
            `**Badges:** ${badges}\n` +
            `**Cor:** ${cor}`,
          inline: true,
        },
...
/**
 * Lista questões do quiz
 */
async function handleListarQuestoes(
  interaction: ChatInputCommandInteraction,
  config: PerfilQuizConfig,
): Promise<void> {
  ...
  config.questions.slice(0, 5).forEach((question, index) => {
    const opcoes = question.answers
      .map((answer: PerfilQuizAnswer, i: number) => `${i + 1}. ${answer.text}`)
      .join("\n");
...
/**
 * Handler para ativar configuração exemplo do quiz de perfil
 */
export async function handleAtivarQuizPerfilExemplo(
  interaction: ChatInputCommandInteraction,
): Promise<void> {
  ...
  if (success) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "A configuração exemplo foi ativada com sucesso!\n\n" +
      ...
      .addFields([
        {
          ...
          value:
            "• Use `/publicar-quiz-perfil publicar` novamente para publicar\n" +
