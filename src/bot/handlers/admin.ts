...
/**
 * Handler principal do painel de administração
 * Exibe o menu de seleção principal com as áreas de configuração
 */
...
/**
 * Exibe o menu principal de administração
 */
async function mostrarMenuPrincipal(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "Bem-vindo ao painel de administração! Selecione a área que deseja configurar abaixo.\n\n" +
        "**Áreas disponíveis:**\n" +
  ...
  if (interaction.isCommand()) {
    ...
    response = (await interaction.message?.edit(payload)) as Message;
  }

  // Coletor de interações do menu principal (sem timeout)
    ...
    try {
      if (componentInteraction.isStringSelectMenu()) {
        if (componentInteraction.customId === "admin_menu_principal") {
          ...
          componentInteraction.customId === "admin_select_funcionalidade"
        ) {
        ...
        if (componentInteraction.customId === "admin_voltar_principal") {
          ...
          // Retorna ao menu de canais usando o último estado conhecido
          await mostrarMenuCanais(interaction as StringSelectMenuInteraction);
        } else if (componentInteraction.customId === "admin_ver_status") {
          await mostrarStatus(interaction as StringSelectMenuInteraction);
        }
      }
    } catch (error) {
...
/**
 * Exibe o menu de configuração de canais com seleção direta
 */
async function mostrarMenuCanais(
  ...
  const formatarCanal = (canalId: string | null | undefined): string =>
    canalId ? `<#${canalId}>` : NAO_CONFIGURADO;

  const embed = new EmbedBuilder()
    .setTitle("🔧 Configuração de Canais")
    .setDescription(
      "**Configure os canais do bot de forma rápida!**\n\n" +
...
/**
 * Handler para seleção de funcionalidade
 */
async function handleSelectFuncionalidade(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
...
async function handleAdminChannelSelect(
  ...
  // Atualizar a configuração de forma tipada
 ...
 * Menu de embeds
 */
async function mostrarMenuEmbeds(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
  const embed = new EmbedBuilder()
    .setTitle("📢 Builder de Embeds")
    .setDescription(
      "Selecione os campos que deseja editar e, em seguida, informe os valores nos modais. Ao final, publique no canal de embeds configurado.",
    )
    .setColor("#5865F2");
  ...
  const seletorCampos =
    new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setMaxValues(5)
        .addOptions(
          { label: "Título", value: "title", emoji: "📝" },
          { label: "Descrição", value: "description", emoji: "📄" },
          { label: "Cor (hex)", value: "color", emoji: "🎨" },
          { label: "Rodapé", value: "footer", emoji: "🔻" },
          { label: "Imagem", value: "image", emoji: "🖼️" },
        ),
    );
  ...
  // Estado do embed em memória durante a sessão
  ...
  collector.on("collect", async (ci) => {
    ...
    if (ci.isStringSelectMenu() && ci.customId === "embed_builder_campos") {
      // Abrir modais em sequência para cada campo
...
async function abrirModalCampo(
  ...
  const input = new TextInputBuilder()
    ...
    .setLabel("Valor")
    .setStyle(
      field === "description" ? TextInputStyle.Paragraph : TextInputStyle.Short,
    )
    .setRequired(true)
  ...
  if (field === "title") draft.setTitle(valor);
  else if (field === "description") draft.setDescription(valor);
  else if (field === "color")
    draft.setColor(
      /^#?[\dA-Fa-f]{6}$/.test(valor) ? `#${valor.replace("#", "")}` : null,
    );
  else if (field === "footer") draft.setFooter({ text: valor });
  else if (field === "image") draft.setImage(valor);
...
/**
 * Menu de quiz de perfil
 */
async function mostrarMenuQuizPerfil(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
  const embed = new EmbedBuilder()
    .setTitle("🎯 Quiz de Perfil")
    .setDescription(
      "**Gerencie o quiz de criação de perfil!**\n\n" +
...
/**
 * Menu de quiz de squads
 */
async function mostrarMenuQuizSquads(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
  const embed = new EmbedBuilder()
 ...
 * Menu de ranking
 */
async function mostrarMenuRanking(
  interaction: StringSelectMenuInteraction,
): Promise<void> {
  const { guildId } = interaction;
  if (!guildId) {
  ...
  const botoes1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
  ...
  const botoes2 = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
    ...
    if (ci.isButton()) {
      if (ci.customId === "rank_toggle") {
        ...
        const menu = new StringSelectMenuBuilder()
          ...
          .addOptions(
            Array.from(canais.values())
              .slice(0, 25)
              .map((c) => {
                // Type assertion: sabemos que são TextChannels pelo filter acima
                const channel = c as { name: string; id: string };
                return {
                  label: channel.name,
                  value: channel.id,
        ...
        const modal = new ModalBuilder()
          ...
          .setTitle(
            isRemove ? "Remover Cargo por Nível" : "Adicionar Cargo por Nível",
          );
...
/**
 * Exibe o status de todas as configurações
 */
...
async function mostrarListarConfig(
  ...
  const formatarCanal = (canalId: string | null | undefined): string =>
    canalId ? `<#${canalId}>` : NAO_CONFIGURADO_BADGE;
