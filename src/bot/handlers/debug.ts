...
export async function handleDebug(interaction: ChatInputCommandInteraction) {
...
async function handleDebugInfo(interaction: ChatInputCommandInteraction) {
...
Cooldowns Ativos: ${statsAtual.activeCooldowns}\`\`\``,
          ...
          },
          {
            ...
            value:
              statsAtual.commandNames.length > 0
          ...
          },
          {
            ...
            value:
              statsAtual.componentIds.length > 0
                ? (() => {
                    const components = `\`${statsAtual.componentIds.slice(0, 10).join("\`, \`")}\``;
                    const extra =
                      statsAtual.componentIds.length > 10
...
Node: ${process.version}\`\`\``,
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "debug_refresh") {
        ...
        await i.deferUpdate();
        // Limpar todos os cooldowns ativos
        ...
        if (global.gc) {
          ...
          await i.followUp({
            content:
              "❌ Garbage collection não disponível. Execute o Node com flag --expose-gc",
            flags: MessageFlags.Ephemeral,
          });
        }
    ...
    collector.on("end", async () => {
      try {
        ...
        // Mensagem já foi deletada ou expirou
      }
    });
  } catch (error) {
...
async function handleDebugDbWrite(interaction: ChatInputCommandInteraction) {
  await interaction.deferReply({ ephemeral: true });
  ...
  try {
    ...
    const result = (await db.get(
      ...
      [testKey],
    )) as { key: string; value: string; updated_at: string } | undefined;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: "💡 Persistência",
          value:
            "Use `/debug db-read` para verificar se o dado persiste após delay",
          inline: false,
        },
      )
...
async function handleDebugDbRead(interaction: ChatInputCommandInteraction) {
  await interaction.deferReply({ ephemeral: true });

  try {
    const db = getDB();

    // Buscar últimos testes de escrita
    ...
    if (results.length === 0) {
      await interaction.editReply({
        content:
          "📭 Nenhum teste de escrita encontrado.\n\n💡 Use `/debug db-write` primeiro!",
      });
      return;
    }
    ...
    for (const [index, result] of results.entries()) {
      const parsedValue = JSON.parse(result.value) as {
        source?: string;
        custom_value?: string;
        test?: string;
      };
      const timeAgo = Math.floor(
        (Date.now() - new Date(result.updated_at).getTime()) / 1000,
      );

      embed.addFields({
        name: `${index + 1}. ${result.key}`,
        value:
          `**Fonte:** ${parsedValue.source || "unknown"}\n` +
