...
import {
  handleAtivarQuizPerfilExemplo,
  handleConfigQuizPerfil,
  handlePublicarQuizPerfil,
} from "@bot/handlers/publicar-quiz-perfil.js";
import {
  ChannelType,
  PermissionFlagsBits,
  SlashCommandBuilder,
...
/**
 * Comando publicar-quiz-perfil - publica e configura o quiz de criação de perfil
 */

export const publicarQuizPerfilCommand: SlashCommand = {
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            ...
            .setRequired(true)
            .addChoices(
              { name: "🔧 Status do Quiz", value: "status" },
              { name: "⚙️ Ativar/Desativar", value: "toggle" },
              { name: "🎨 Cor Padrão", value: "cor" },
              { name: "📋 Listar Resultados", value: "resultados" },
              { name: "❓ Listar Questões", value: "questoes" },
            ),
        )
        .addStringOption((opt) =>
          opt
