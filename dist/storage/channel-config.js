import { defaultChannelConfig } from "../tipos/index.js";
import { getDB } from "./db-connector.js";
function isChannelConfig(data) {
...
export async function getChannelConfig(guildId) {
    try {
        ...
        const row = (await db.get("SELECT * FROM channel_config WHERE guild_id = $1", [guildId]));
        if (!row)
            return defaultChannelConfig(guildId);
        const config = {
            ...
            xp_channels: row.xp_channels
                ? JSON.parse(row.xp_channels)
                : [],
            xp_ignore_channels: row.xp_ignore_channels
                ? JSON.parse(row.xp_ignore_channels)
                : [],
            ...
            bot_commands_channels: row.bot_commands_channels
                ? JSON.parse(row.bot_commands_channels)
                : [],
        ...
        console.warn(`[DB] Erro ao buscar configuração de canais para guild ${guildId}:`, error instanceof Error ? error.message : String(error));
        return defaultChannelConfig(guildId);
    }
}
export async function setChannelConfig(config) {
    try {
        const db = getDB();
        if (!db) {
            console.error("[DB] Banco de dados não disponível");
    ...
    `;
        ...
        console.log(`[DB] Configuração de canais salva para guild ${config.guild_id}`);
        return true;
    }
    catch (error) {
        console.error(`[DB] Erro ao salvar configuração de canais para guild ${config.guild_id}:`, error instanceof Error ? error.message : String(error));
        return false;
    }
}
export async function isChannelAllowed(guildId, channelId, feature) {
    const config = await getChannelConfig(guildId);
    switch (feature) {
        case "embeds_channel":
            return !config.embeds_channel || config.embeds_channel === channelId;
        case "xp_channels":
            if (config.xp_ignore_channels?.includes(channelId))
                return false;
            return (!config.xp_channels?.length || config.xp_channels.includes(channelId));
        case "perfil_quiz_channel":
            return (!config.perfil_quiz_channel || config.perfil_quiz_channel === channelId);
        case "squad_quiz_channel":
            return (!config.squad_quiz_channel || config.squad_quiz_channel === channelId);
        case "admin_commands_channel":
            return (!config.admin_commands_channel ||
                config.admin_commands_channel === channelId);
        case "bot_commands_channels":
            if (!config.restrict_commands)
                return true;
            return (!config.bot_commands_channels?.length ||
...
export async function getConfiguredChannel(guildId, feature) {
    const config = await getChannelConfig(guildId);
    switch (feature) {
        case "embeds_channel":
...
export async function removeChannelConfig(guildId, feature) {
    try {
        const config = await getChannelConfig(guildId);
        switch (feature) {
            case "embeds_channel":
