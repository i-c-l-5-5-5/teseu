import { checkChannelPermission } from "../core/channel-middleware.js";
...
export async function handleRank(interaction) {
    try {
        ...
        const gerarEmbed = () => {
            ...
            const barraProgresso = "▰".repeat(Math.floor(progresso * 10)) +
                "▱".repeat(10 - Math.floor(progresso * 10));
            return new EmbedBuilder()
                .setTitle(`🏅 Rank de ${displayName}`)
                .setThumbnail(avatar)
                .setColor("#FFD700")
                .addFields({ name: "Nível", value: String(level), inline: true }, { name: "XP Total", value: String(xp), inline: true }, {
        ...
        collector.on("collect", async (i) => {
            ...
            if (i.customId === "rank_refresh") {
                ...
                await i.reply({
                    content: "🏆 Top 10 em desenvolvimento! Em breve você verá o ranking completo do servidor.",
                    flags: MessageFlags.Ephemeral,
                });
            }
        });
