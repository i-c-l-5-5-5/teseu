...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, EmbedBuilder, MessageFlags, } from "discord.js";
import { resumoConfiguracoesEmbed } from "./utils/config-helpers.js";
export async function handleAdminSelectFuncionalidade(interaction) {
...
export async function handleAdminChannelSelection(interaction) {
...
async function mostrarListarConfig(interaction) {
    ...
    const config = await getChannelConfig(guildId);
    const embed = resumoConfiguracoesEmbed(config).setTitle("📋 Todas as Configurações");
    const botaoVoltar = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId("admin_voltar_canais")
        .setLabel("⬅️ Voltar")
        .setStyle(ButtonStyle.Secondary));
